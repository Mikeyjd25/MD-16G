...
def main(in_file, out_file, enable_print=True):
    def reg(reg_id):
        if reg_id == "GP0":
            return "00000"
        elif reg_id == "GP1":
    ...
    def add_goto(goto_id):
        goto_list[goto_id] = format(bin_count[0], '016b')

    start_time = int(round(time.time() * 1000))
    bin_count = {0: 0}  # Keeps track of how long program is
    ...
    if(enable_print):
        print("Assembled Size: ["+str(program_len)+"/65536]")
    end_time = int(round(time.time() * 1000))

    if(enable_print):
        print("Assembly Took: " + str(end_time-start_time) + "ms")
